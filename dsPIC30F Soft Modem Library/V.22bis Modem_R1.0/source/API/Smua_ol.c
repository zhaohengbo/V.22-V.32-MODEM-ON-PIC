...
INT main(void)
{
	...
	{
    	home_clr();
    	puts_lcd( " Embedded Modem ", 16 );
    	line_2();
    	puts_lcd( " Demo-> Ready   ", 16 );

		if ( SM_Config_Params.SMMode == SM_ORIGINATE )		
			/* start modem upon detecting S1 closure */
			while ( PORTAbits.RA12 );
			
		line_2();
    	puts_lcd( " Demo-> Running ", 16 );
...
UCHAR Poll_SMUA_OL_Status(void)
{
    ...
    if( SMConnect != 0 )
    {
        if( InDataW == InDataR )
	    {
//			ADCON1bits.SAMP = 0;  // start conversion sequence
			ADCON1bits.ADON = 1;
		    DummyTask();
//		    ADCON1bits.ADON = 0;
	    }
